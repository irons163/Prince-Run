...
public class PlatformFactory extends Layer{
	...
	public PlatformFactory(int w, int h, boolean autoAdd) {
		...
		textureLeft = GraphicsUtils.loadImage("assets/prince_run/animation/Hero01/Jump/platform_l.png");
	    textureMid = GraphicsUtils.loadImage("assets/prince_run/animation/Hero01/Jump/platform_m.png");
	    textureRight = GraphicsUtils.loadImage("assets/prince_run/animation/Hero01/Jump/platform_r.png");
	}
  ...
  //平台向左移动的方法
    public void move(float speed, Panda panda){
        for(Sprite p : platforms){
        	if(!p.isHidden() && panda.getFrame().top + panda.getHeight() > p.getFrame().top && panda.getFrame().top < p.getFrame().top + p.getHeight() && p.getX() - speed <= panda.getX() + panda.getWidth() && p.getX() + p.getWidth() >= panda.getX()){
